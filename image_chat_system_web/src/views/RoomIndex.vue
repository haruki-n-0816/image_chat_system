  ...
  <div>
    ...
    <ul v-if="chatIndexes.length > 0">
      ...
      <li v-for="chatIndex in chatIndexes" :key="chatIndex.roomId">
        <div><router-link :to="{ path: '/chatpage/' + chatIndex.roomId }">{{ chatIndex.roomName + "の部屋"}}</router-link></div>
      </li>
    </ul>
    <div v-else>
      いまは部屋がありません...
